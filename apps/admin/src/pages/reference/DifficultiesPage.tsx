...
export default function DifficultiesPage() {
  ...
  const handleSaveValue = async (item: Difficulty) => {
    ...
    try {
      setSavingId(item.id);
      await updateDifficulty(item.id, { value });
      await load();
  ...
  const handleMove = async (index: number, direction: -1 | 1) => {
    ...
    try {
      await Promise.all(
        reordered.map((item, i) => updateDifficulty(item.id, { sortOrder: i }))
      );
      await load();
    } catch {
  ...
  const handleDelete = async (item: Difficulty) => {
    const confirmed = await confirm({
      title: 'Удалить значение?',
      message: `Удалить "${item.value}" из списка сложностей?`,
      type: 'danger',
    });
    if (!confirmed) return;

    try {
      await deleteDifficulty(item.id);
      await load();
  ...
  return (
    <div className={styles.page}>
      <div className={styles.header}>
        <h1 className={styles.title}>Сложности</h1>
      </div>
      <p className={styles.subtitle}>
        Эти значения доступны в выпадающем списке «Сложность» в форме квеста.
