...
export default function ShowProgramForm() {
  ...
  const [formData, setFormData] = useState<CreateShowProgramData>({
    name: '',
    priceRub: 0,
    supplierId: null,
    imageId: null,
  ...
  const loadShowProgram = async () => {
    try {
      setLoading(true);
      const item = await getShowProgram(id!);
      setFormData({
        name: item.name,
      ...
      setError('Ошибка загрузки шоу-программы');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className={styles.page}>
      <div className={styles.header}>
        <h1 className={styles.title}>{isEdit ? 'Редактировать шоу-программу' : 'Новая шоу-программа'}</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className={styles.form}>
        ...
        <div className={styles.actions}>
          <button type="button" className={styles.cancelButton} onClick={() => navigate('/content/show-programs')}>
            Отмена
          </button>
